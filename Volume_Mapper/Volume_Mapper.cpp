...
namespace {
	...
	// CC 号
	const int kCcVolume = 7;           // CC7 通道音量
	...
	struct RawData {
		...
		bool hasRealEvent = false;    ///< 文件里是否真有 CC7（补出来的默认值不算）
		int  realCount = 0;           ///< 真正读到的 CC7 条数（补全之前统计）
...
namespace VOL {
	...
	Result VM(const std::string& path)
	{
		...
		r.ok = true;
		r.message = "Volume：通道 " + std::to_string(r.curve.channel)
		          + "，" + std::to_string(realEvents) + " 个 CC7 事件"
	...
	int ValueAt(const Curve& curve, double quarters)
	{
		...
		if (it == curve.points.begin())
			return kDefaultValue;          // 第一个事件之前：默认通道音量
		return (it - 1)->value;
	}
