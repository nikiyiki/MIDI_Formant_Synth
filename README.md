...
排障经验与已知坑见上文「已知限制」；本仓库暂不含独立的排障手册。
...
面向**使用者**的手册（怎么用、每个参数什么意思、出问题怎么办）以发布包里的 `OM.pdf` 形式分发，
源码在其 HTML 源文件里维护。详细规格见发布包内的 `spec1.0.2.pdf`。

> 本仓库当前不含内部技术文档（技术协议原文、音素字典说明、输出规格与决策记录、排障手册）。
> 其中描述的实现依据已在上文各节就地引用；这些文档整理后会另行发布。
